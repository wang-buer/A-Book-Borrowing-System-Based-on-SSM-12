...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">挂失信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>挂失信息管理</span></li>
								<li class="breadcrumb-item active"><span>挂失信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">挂失信息列表</h3>							
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var guashineirongCell  = document.createElement('td');

						  if(item.guashineirong != null && item.guashineirong != "" && item.guashineirong.length >= 11){
			  	guashineirongCell.innerHTML = item.guashineirong.substring(0, 10) + "..."; 
			  }else{
			  	guashineirongCell.innerHTML = item.guashineirong;
			  }


			row.appendChild(guashineirongCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("guashixinxi/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("guashixinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("guashixinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"guashixinxi");
